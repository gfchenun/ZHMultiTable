...
@interface ZHMultiTablePage ()
<UIScrollViewDelegate> {
    struct {
        CGFloat headHeight; // 头部高度
        CGFloat bottomInset; // 底部间隔
        CGFloat minHeadFrameOriginY; // Y轴最小位置
    } _headParameter;  // 头部参数
    
    struct {
        BOOL tabViewLoadFlag;  // 是否加载了tableview
        BOOL pluginsLoadFlag;  // 是否加载了插件
    } _loadParameter; // 加载参数
    
    CGFloat  _contentOffsetY;  // 当前Y坐标
    BOOL     _headViewScrollEnable; // 是否记过headerView的滑动
    BOOL     _viewDidAppearIsCalledBefore; // 是否在加载完成之前viewDidAppear
}
...
@implementation ZHMultiTablePage

#pragma mark - system
- (void)dealloc {
    [self removeKVOObserver];
    for (ZHMultiTablePagePlugin_Base *plugin in self.plugins.objectEnumerator) {
        [plugin removePlugin];
    }
...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    [self.plugins enumerateObjectsUsingBlock:^(ZHMultiTablePagePlugin_Base *plugin, NSUInteger idx, BOOL *stop) {
        [plugin zh_scrollViewWillScrollFromIndex:self.curIndex];
    }];
...
- (void)tabDelegateScrollViewVerticalScroll:(float)percent {
    ...
    [self.plugins enumerateObjectsUsingBlock:^(ZHMultiTablePagePlugin_Base *plugin, NSUInteger idx, BOOL *stop) {
        [plugin zh_scrollViewVerticalScroll:percent];
    }];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self.plugins enumerateObjectsUsingBlock:^(ZHMultiTablePagePlugin_Base *plugin, NSUInteger idx, BOOL *stop) {
        [plugin zh_scrollViewHorizontalScroll:scrollView.contentOffset.x];
    }];
}
...
- (void)viewDidScrollToIndex:(NSInteger)index {
    ...
    [self.plugins enumerateObjectsUsingBlock:^(ZHMultiTablePagePlugin_Base *plugin, NSUInteger idx, BOOL *stop) {
        [plugin zh_scrollViewDidScrollToIndex:index];
    }];
}
#pragma mark - Plugin
...
- (void)enablePlugin:(ZHMultiTablePagePlugin_Base *)plugin {
    ...
    [self.plugins enumerateObjectsUsingBlock:^(ZHMultiTablePagePlugin_Base *existPlugin, NSUInteger idx, BOOL *stop) {
